...
export MINIO_REFRESH_IAM_INTERVAL=60s
...
test_user_management()
{
    ...
    sleep 65
    user=$(./mc admin user list gateway2 | grep user1)
    if [ -z "$user" ]
    then
    ...
    sleep 65
    user=$(./mc admin user list gateway1 | grep user1)
    if [ ! -z "$user" ]
    then
...
test_group_management()
{
    ...
    result=$(./mc admin group info gateway1 testcents ｜grep Members |awk '{print $2}')
...
test_mult_gateways_set_group()
{
    ...
    sleep 65
    result=$(./mc admin group info gateway2 testcents ｜grep Members |awk '{print $2}')
    if [ "$result" != "user1,user2,user3" ]
    then
      echo "error,result is '$result'"
      exit 1
    fi
    ./mc admin group enable gateway testcents
...
test_user_svcacct_add()
{
    ...
    start_two_gateway
    ./mc admin user add gateway1 user1 admin123
    ./mc admin policy set gateway consoleAdmin user=user1
    ./mc alias set gateway1_user1 http://127.0.0.1:9001 user1 admin123
...
test_user_sts()
{
    ...
    cd minio
    go run docs/sts/assume-role.go -sts-ep http://127.0.0.1:9001 -u user1 -p admin123 -d
    go run docs/sts/assume-role.go -sts-ep http://127.0.0.1:9001 -u user1 -p admin123
