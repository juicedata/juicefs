...
kill_gateway() {
    port=$1
    netstat -ano | findstr "$port" | findstr "LISTENING" | awk '{print $5}' | xargs -r taskkill /F /PID
}
...
start_two_gateway()
{
    ...
    sleep 1
    nohup ./juicefs.exe gateway $META_URL 127.0.0.1:9002 --multi-buckets --keep-etag --object-tag -background --log=gateway2.log &
    sleep 2
...
test_user_management()
{
    ...
    if ./mc.exe cp mc gateway1_user1/test1/file1
    then
...
test_user_svcacct_add()
{
    ...
    if ./mc.exe cp mc svcacct1/test1/file1
    then
...
test_user_sts()
{
    ...
    ./mc.exe cp mc gateway1_user1/test1/mc
    cd minio
...
test_change_credentials()
{
    ...
    start_two_gateway
    ./mc.exe mb gateway1/test1
    ./mc.exe cp mc gateway1/test1/file1
    ...
    ./juicefs.exe gateway $META_URL 127.0.0.1:9001 --multi-buckets --keep-etag --object-tag -background
    ./juicefs.exe gateway $META_URL 127.0.0.1:9002 --multi-buckets --keep-etag --object-tag -background
    sleep 5
