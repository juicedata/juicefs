...
func changelogApply(ctx *cli.Context) error {
	...
	if !dryRun {
		logger.Infof("Rebuilding destination volume stats before applying changelogs")
		if err := meta.RebuildVolumeStats(applyCtx, dst); err != nil {
			return fmt.Errorf("rebuild destination volume stats: %w", err)
		}
	}
