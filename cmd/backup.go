...
func cmdStatBak() *cli.Command {
	return &cli.Command{
		Name:      "stat",
		Action:    statBak,
		Category:  "ADMIN",
		Usage:     "show metadata backup stats (Only supports uncompressed file)",
		ArgsUsage: "FILE",
	}
}
...
func dumpBak(ctx *cli.Context) error {
	setup(ctx, 1)
...
func loadBak(ctx *cli.Context) error {
	setup(ctx, 1)
	metaUri := ctx.Args().Get(0)
	src := ctx.Args().Get(1)
	removePassword(metaUri)
