...
func gc(ctx *cli.Context) error {
	...
	r := m.ScanSlices(c, &meta.ScanSlicesOption{ScanPending: true, Delete: delFlag, Progress: sliceCSpin.Increment}, func(ino meta.Ino, s meta.Slice) error {
...
func gcExternalSort(
	...
	eg.Go(func() error {
		...
		st := m.ScanSlices(c, &meta.ScanSlicesOption{ScanPending: true, Progress: metaSliceSpin.Increment}, func(ino meta.Ino, s meta.Slice) error {
			select {
			case <-egCtx.Done():
				return egCtx.Err()
			default:
