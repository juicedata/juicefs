...
- 只能使用同一对象存储下的多个 bucket
- 访问对象存储的 API Key 要有创建 bucket 的权限
- 需要使用整型数字通配符 `%d` 指定 bucket 例如 `"http://192.168.1.18:9000/myjfs-%d"`，可以按照这样的格式预先创建 bucket，也可以在创建文件系统时由 JuiceFS 客户端自动创建；
- 如果使用预先手动创建的 bucket 用于 shards，这些被指定的 bucket 必须是空的；
