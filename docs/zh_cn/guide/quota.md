...
- 目录用量：目录配额使用量（`juicefs quota --path ...`、`summary`/`info` 相关目录统计）。
- 用户/组用量：用户配额与组配额使用量（`--uid` / `--gid`）。
...
| 回收站文件 | 移入回收站后仍计入；仅在回收站真正清理后释放 | 从原目录树移出后，不再计入原目录/祖先目录；且不支持对回收站目录设置目录配额 | 移入回收站后仍计入原属主 UID/GID；仅在回收站真正清理后释放 |
...
- 对目录用量：对象从原目录树移出后，原目录配额用量会下降；回收站目录暂不支持设置目录配额。
