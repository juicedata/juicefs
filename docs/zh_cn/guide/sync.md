...
类似 rsync，你可以用 `--exclude`、`--include` 来过滤需要同步的文件，并通过多个规则的组合实现任意集合的同步。
要想熟练使用这两个参数，首先需要明确意义 `include` 和 `exclude`参数的含义，默认情况下，juicefs 默认是传输所有扫描到的文件的，但是 exclude 参数则允许排除符合某种模式的文件从而达到某些文件不被传输的效果。而 `include` 与`exclude` 的含义相反则是不排除这些文件。
juicefs sync 会对每一个名称检查是否能匹配 incluee/exclude 规则，且先匹配上的规则生效：如果能匹配上exclude规则，则该文件被跳过，如果该文件能匹配 include 或不能匹配任何规则，则不跳过该文件。

### 模式匹配规则

+ 如果匹配模式以斜线(/)开头，它表示锚定层次结构中某个特定位置的文件，否则将表示匹配路径名的结尾。这有点类似于正则表达式中的行首"^"。因此"/foo"匹配的是传输中根目录的"foo"文件。如果匹配模式是 "foo" ，由于算法会从顶部开始向下深度优先递归，所以它能匹配任意位置名为"foo"的文件。但是对于非锚定的"sub/foo"模式，将匹配层次结构中位于sub目录下的foo。

+ 若匹配模式以斜线(/)结尾，将只匹配目录，而不匹配普通文件、字符链接以及设备文件。

+ Juicefs 通过检查匹配模式中是否包含`*`、"?"以及"["符号来决定做简单的字符串匹配还是通配符匹配。

+ 单个`*`匹配任意路径元素，但在遇到斜线时终止匹配。

+ `**`匹配任意路径元素，与`*`不同的是，它能匹配斜线。
...
+ 如果匹配模式中包含了一个"/"(不包括以斜线结尾的情况)或`**`，则表示对包括前导目录的全路径进行匹配。如果匹配模式中不包括"/"或`**`，则表示只对全路径尾部的路径元素进行匹配。(注意：使用了递归功能时，"全路径"可能是从最顶端开始向下递归的某中间一段路径)。

+ 对于`dir_name/***`来说，它将匹配 dir_name 下的所有层次的文件。
  注意，每个子路径元素会自顶向下逐层，被访问因此include/exclude匹配模式会对每个子路径元素的全路径名进行递归(例如，要包含"/foo/bar/baz"，则"/foo"和"/foo/bar"必须不能被排除)。实际上，排除匹配模式在发现有文件要传输时，此文件所在目录层次的排除遍历会被短路。如果排除了某个父目录，则更深层次的include模式匹配将无效，因为Juicefs从排除的那个父目录位置开始不会再向下遍历。这在使用尾随"*"时尤为重要。例如，下面的例子不会正常工作：
...
+ 同时使用`--include */ --include *.c --exclude *` 将只包含所有目录和C源码文件，除此之外的所有文件和目录都被排除。
