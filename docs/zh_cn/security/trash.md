...
开启回收站功能后，可以通过chattr命令为文件或目录设置's'属性，带有's'属性的文件或目录在被删除时不会进入回收站，而是直接从文件系统中移除。如果父目录设置了's'属性，则该目录下新创建的文件和子目录都会继承该标志。为一个目录设置's'属性，已存在的和之后转移过来的文件或目录不会继承这个属性。

需要在挂载时启用`--enable-ioctl`选项，才能使用chattr命令修改文件属性。
