...
1. **数据加密层**（对称加密 - AES-GCM-256）
   - **作用**：实际加密用户数据内容
   - **机制**：每个 4MB 数据块生成唯一的 256 位对称密钥 `S` + 随机种子 `N`
   - **优势**：AES-GCM 提供高速加密和完整性验证（AEAD）
   - **标准**：256 位密钥强度符合 NIST 安全标准
...
值得一提的是，加密后的数据会比原始数据稍大一些，主要是因为 AES-GCM 加密算法需要添加认证标签（16 字节）和其他加密元数据。不过在现代网络环境下，这点额外的传输量通常不会成为问题。
