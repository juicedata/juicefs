...
JuiceFS 的静态数据加密采用了行业标准的加密算法组合：对称加密支持 AES-GCM 和 ChaCha20-Poly1305，非对称加密使用 RSA。只需在创建文件系统时提供一个 RSA 私钥即可为文件系统启用数据加密功能，通过 `JFS_RSA_PASSPHRASE` 环境变量提供私钥密码。在使用上，挂载点对应用程序完全透明，即加密和解密过程对文件系统的访问不会产生影响。
...
1. **数据加密层**（对称加密 - AES-GCM-256 或 ChaCha20-Poly1305）
   - **作用**：实际加密用户数据内容
   - **机制**：每个 block 生成唯一的对称密钥 `S` + 随机种子 `N`（AES-GCM 使用 256 位密钥，ChaCha20 使用 256 位密钥）
   - **优势**：AES-GCM 和 ChaCha20-Poly1305 都提供高速加密和完整性验证（AEAD）
...
值得一提的是，加密后的数据会比原始数据稍大一些，主要是因为 AES-GCM 和 ChaCha20-Poly1305 加密算法需要添加认证标签（16 字节）和其他加密元数据。不过在现代网络环境下，这点额外的传输量通常不会成为问题。
...
在部署规划时，请考虑到加密会带来额外的 CPU 和内存开销，加密后的数据体积也会略有增加（主要来自加密算法所需的认证标签和元数据）。为了确保最佳的兼容性和稳定性，建议所有访问加密文件系统的客户端使用相同或兼容版本的 JuiceFS。
...
- **移动设备或嵌入式系统**：优先选择 ChaCha20-Poly1305

这些组合在安全性和性能之间取得了很好的平衡。记得定期评估您的加密策略，确保它始终符合您的安全要求和业务需求。
:::
