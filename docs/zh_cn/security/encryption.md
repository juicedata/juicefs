...
1. **数据加密层**（对称加密 - AES-GCM-256）
   - **作用**：实际加密用户数据内容
   - **机制**：每个4MB数据块生成唯一的256位对称密钥 `S` + 随机种子 `N`
   - **优势**：AES-GCM提供高速加密和完整性验证（AEAD）
   - **标准**：256位密钥强度符合NIST安全标准

2. **密钥保护层**（非对称加密 - RSA）
   - **作用**：保护对称密钥的安全分发和存储
   - **机制**：使用RSA私钥 `M` 加密每个数据块的对称密钥 `S`
   - **优势**：解决密钥分发难题，避免密钥重用风险
   - **方案**：支持RSA-2048或RSA-4096密钥，采用OAEP填充增强安全性
...
启用加密功能确实会带来一定的性能开销，但现代硬件技术已经让这种影响变得相当可控。具体的性能影响取决于工作负载类型、硬件配置（特别是CPU的加密指令集支持）和数据访问模式。
...
值得一提的是，加密后的数据会比原始数据稍大一些，主要是因为 AES-GCM 加密算法需要添加认证标签（16字节）和其他加密元数据。不过在现代网络环境下，这点额外的传输量通常不会成为问题。
