...
ZeroFS 核心服务同时处理元数据、数据以及所有协议端点，因此不需要额外运行一个单独的元数据数据库。它面向的是希望以合理运维成本自托管、基于对象存储的文件访问能力的团队，而不是需要横向扩展海量吞吐量的团队。关于其单
Leader 设计和强制加密如何影响这一权衡，详见下文的[系统架构](#architecture)部分。

**JuiceFS** 是一款云原生分布式文件系统，专为跨多云环境下高要求的 AI/ML
训练、高性能计算和大数据分析场景设计。通过采用[数据与元数据分离](../architecture.md)的架构，并搭配可插拔、可独立扩展的元数据引擎，JuiceFS
...
| Kubernetes CSI | ✓ 支持（RWO/ROX/RWX）                                   | ✓ 支持                                         | ✓ 支持                                      |
