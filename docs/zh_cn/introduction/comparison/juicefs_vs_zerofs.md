...
**ZeroFS** 支持多个客户端并发写入：其 Kubernetes CSI 驱动提供真正的 ReadWriteMany 卷，任意节点上的任意 Pod 均可通过 9P
协议挂载，且 POSIX 字节范围锁由 Leader 统一仲裁。不过，ZeroFS 的扩展性限制位于服务端，而非客户端。每次写入必须先经由唯一的
Leader 进程才能最终写入对象存储，而该进程无法横向扩展。HA 节点对仅用于故障转移，不提供写入方向的水平扩展；若 Leader
成为瓶颈，只能升级为性能更强的单机，无法通过增加 Leader 数量解决。Standby 节点作为热备份保持同步，以实现高可用。而只读副本节点（Read
Replicas）则以有限的数据滞后（写入方刷新间隔，默认 30 秒，再加上最多 10 秒的副本轮询窗口）提供一定的最终一致的读取扩展性，但不能增加写入能力。
...
| 数据存储       | AWS S3、兼容 S3 的存储、Google Cloud Storage、Azure Blob | 任意主流对象存储及其他类型存储                  | 任意主流对象存储及其他类型存储               |
| 数据冗余保护   | 由对象存储提供                                           | 由数据存储层提供                                | 由数据存储层提供                             |
| 数据缓存       | 本地磁盘 + 内存（双层缓存）                              | 本地缓存                                        | 内存 + 本地缓存 + 分布式缓存                 |
