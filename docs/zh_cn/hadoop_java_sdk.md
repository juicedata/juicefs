...
| `juicefs.accesskey`              |                              | 对象存储的访问 ID（Access Key ID）。如果计算节点已经有访问对象存储的权限，则无需提供。请查看[这个文档](how_to_setup_object_storage.md)了解如何获取不同对象存储的 access key。                          |
| `juicefs.secretkey`              |                              | 对象存储的私钥 (Secret Access Key)。如果计算节点已经有访问对象存储的权限，则无需提供。请查看[这个文档](how_to_setup_object_storage.md)了解如何获取不同对象存储的 secret key。                          |
