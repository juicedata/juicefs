...
二进制备份可能包含重复记录，例如 Redis `SCAN` 多次返回同一个 key。将节点、目录项、chunk、符号链接和扩展属性加载到 SQLite、MySQL 或 PostgreSQL 时，JuiceFS 会跳过重复键，并为发生跳过的每个批次记录提交数、插入数和跳过数。加载过程保留已有记录，不比较内容；并发加载不保证保留备份中的哪一次记录。MySQL 使用 `INSERT IGNORE`，还可能将部分非法值转换为警告。

导入 JSON 备份时，JuiceFS 会重新计算空间使用量、inode 计数器等统计信息。导入二进制备份时，如果备份标明来源为 Redis，则保留备份中的计数器；SQL/KV 来源或没有来源信息的旧备份，则根据恢复的记录重建计数器。该策略适用于所有目标引擎，包括 Redis。如果你对 JuiceFS 的元数据设计有深入理解，还可以在恢复前对元数据备份文件进行修改，以此来进行调试。

二进制恢复也支持管道输入，读取并验证输入末尾的 footer 后才会写入计数器。如果 footer 损坏或导入失败，目标库中可能已经写入部分元数据，恢复操作不会回滚这些记录；重试时应使用空的目标库。
