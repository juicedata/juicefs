...
JuiceFS 的 sync 子命令是功能完整的数据同步实用工具，可以在所有 [JuiceFS 支持的对象存储](../reference/how_to_setup_object_storage.md)之间多线程并发同步或迁移数据，既支持在“对象存储”与“JuiceFS”之间迁移数据，也支持在“对象存储”与“对象存储”之间跨云跨区迁移数据。与 rsync 类似，支持本地目录、通过 SSH 访问远程目录，还支持 HDFS 、WebDAV 等，提供全量同步、增量同步、条件模式匹配等高级功能。
...
sync 命令的模式匹配功能跟 rsync 类似，可以通过规则排除或包含某类文件，通过多个这种规则的组合实现任意集合的同步，规则如下：
...
特别地，以 `/` 结尾将被视为目录，例如：`movies/`。没有以 `/` 结尾则会被视为 `prefix` 前缀，它将按照前缀的规则进行匹配，例如，当前目录下有 `test` 和 `text` 两个目录，使用以下命令可以将它们同步到目标路径 `~/mnt/`：
