...
JuiceFS 默认会分配 100GB 的缓存空间，但这并不代表你要使用超过该容量的磁盘。这个值代表的是在磁盘容量允许的情况下 JuiceFS 客户端可能使用的最大容量。当磁盘容量低于该值时，JuiceFS 会保证缓存容量始终低于磁盘容量的 10%。

例如，你将 `--cache-dir` 设置在了容量为 50GB 的分区，此时，不论 `--cache-size` 设置多大，JuiceFS 的缓存容量始终保持在 45GB 左右，即保留该分区 10% 的剩余空间。
