...
当访问 JuiceFS 中的文件时，会有多级缓存给经常访问的数据提供更好地性能，读请求会依次尝试内核分页缓存、JuiceFS进程的预读缓冲区、本地磁盘缓存，当缓存中没找到对应数据时才会从对象存储读取，并且会异步写入各级缓存保证下一次访问的性能。
...
当需需要同时缓存多个目录时，可以将所有路径写入一个文本文件。例如，创建一个名为 `warm.txt` 的文本文件，每行一个挂载点中的路径：
...
JuiceFS 默认会分配 100GiB 的缓存空间，但这并不代表你要使用超过该容量的磁盘。这个值代表的是在磁盘容量允许的情况下 JuiceFS 客户端可能使用的最大容量。当磁盘的剩余空间不足 100 GiB 时，保证剩余空间不低于 10%。

例如，你将 `--cache-dir` 设置在了容量为 50GiB 的分区，此时，不论 `--cache-size` 设置多大，JuiceFS 的缓存容量始终保持在 45GiB 左右，即保留该分区有 10% 以上的剩余空间。

当设定的缓存空间被写满时，JuiceFS 客户端会采用类 LRU 模式对缓存进行清理，即尽量清理较早且较少使用的缓存。
