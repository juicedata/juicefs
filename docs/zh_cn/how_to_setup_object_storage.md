...
- `--access-key` 和 `--secret-key` 用来指定访问对象存储的身份认证密钥，需要在相应云平台上创建。如果使用的机器上已经通过环境变量或者其他方式获得了访问对象存储的权限，它们可以不提供。
