...
|`--readdir-cache=false` <VersionAdd>1.3</VersionAdd>|开启目录项缓存, 默认为false，代表不开启|
|`--negative-dir-entry-cache=0` <VersionAdd>1.3</VersionAdd>|失败lookup查询 (返回 ENOENT) 缓存过期时间, 默认为0，代表不缓存|
