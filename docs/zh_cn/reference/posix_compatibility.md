...
- JuiceFS 版本：1.3.0-dev+2025-06-06
- LTP 版本：20260529
...
5. 创建测试配置文件。为方便测试，从 `runtest/fs` 和 `runtest/syscalls` 中删去压力测试、与文件系统不相关的条目以及不在 JuiceFS 上运行的测试（参见[附录](#附录)，修改后保存到文件 `fs-jfs` 和 `syscalls-jfs`）：

   ```bash
   # 生成 fs-jfs：从 fs 中删除压力测试和无关条目
   sed -e '/^gf01 /d' -e '/^gf02 /d' ... runtest/fs > runtest/fs-jfs

   # 生成 syscalls-jfs：从 syscalls 中删除与文件系统无关的条目
   cp runtest/syscalls runtest/syscalls-jfs
   sed -i -e '/^bpf_prog05 /d' ... runtest/syscalls-jfs

   # 删除不在 JuiceFS 上运行的测试（使用 .all_filesystems 机制，在其他本地文件系统上运行）
...
| **合计** | **1523** | **18799** | **0** | **0** | **314** |

所有在 JuiceFS 上运行的测试均通过，0 失败，0 Broken。
