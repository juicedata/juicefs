...
加载过程中 `juicefs load` 会自动处理好因包含不同时间点文件而产生的冲突问题，并重新计算文件系统的统计信息（空间使用量，inode 计数器等），最后在新数据库中生成一份全局一致的元数据。如果元数据内容有异常，还可以在 load 前手动修改 JSON 文件（请务必小心）以尝试修复问题。

## 元数据迁移

将通过 `juicefs dump` 导出的 JSON 内容导入到另一个引擎的**空数据库**中，即实现了元数据离线迁移功能，如：
