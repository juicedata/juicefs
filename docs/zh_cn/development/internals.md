...
- Block：一个 Chunk 包含一个或多个 Block，这也是文件分割后的实际最小存储单位，默认大小 4M。JuiceFS Client 会并发读取 Block，大大提高文件的读性能。除此外，Block 也是本地缓存的基本单位，当缓存换出发生时，置换效率也会因此提高。除此外，Block 一旦生成后就不可变，更新文件会以新增 Block 的形式发生：新的 Block 上传到对象存储，然后对应的文件元数据将会在 Slice 列表里追加新的 Slice；
- Slice：数据写入的逻辑单位，如果说 Block 的存在是为了优化读，那么 Slice 就是为了优化覆盖写、随机写场景。所有的写入都会视情况落到已有或者新创建的 Slice 上，而读取文件时，实际上是在读取通过合并 Slice 列表而得到的数据视图。详见 [JuiceFS 读写请求处理流程](../introduction/io_processing.md)。
