...
## Mount errors due to permission issue
...
This only occurs when a non-root user is trying to run `juicefs mount`, meaning `fusermount` is not found, there are two solutions to this problem:
...
To avoid this type of issue, we recommend disabling background jobs on low-bandwidth clients, i.e. adding [`--no-bgjob`](../reference/command_reference.md#mount) to the mount command.
...
* JuiceFS mount client is an Go program, which means you can decrease `GOGC` (default to 100, in percentage) to adopt a move active garbage collection. This inevitably increase CPU usage and may even directly hinder performance. Read more in [Go Runtime](https://pkg.go.dev/runtime#hdr-Environment_Variables).
