...
When using Redis Cluster, the database number specified in the connection URL is used as a **key prefix** rather than for actual database selection (since Redis Cluster only supports database 0). The prefix format is `{N}` (e.g., `{1}`, `{2}`), which uses Redis hash tags to ensure that all keys belonging to the same volume are routed to the same hash slot. You can also specify an explicit prefix with `?prefix=<value>`; JuiceFS will automatically wrap it in hash tags when running in cluster mode. This allows multiple JuiceFS file systems to share a single Redis Cluster:
...
- To add a prefix to all JuiceFS metadata keys stored in a Redis database, include `?prefix=<value>` in the metadata URL. In Redis Cluster, the prefix is automatically converted into a Redis hash tag so all keys stay in the same slot. Avoid using Redis glob metacharacters such as `*`, `?`, and `[` in custom prefixes.
