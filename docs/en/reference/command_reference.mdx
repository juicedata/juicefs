...
|Items|Description|
|-|-|
|`--files-from` <VersionAdd>1.3</VersionAdd>|Only synchronize the objects recorded in the given file, where each line is the relative path of the object to be synchronized. If the object is a directory, it must end with a /.|
