#### Metadata related options {#metadata-options}
...
#### Metadata cache related options {#metadata-cache-options}
...
#### Data storage related options {#data-storage-options}
...
#### Data cache related options {#data-cache-options}
...
|`--cache-expire=0` <VersionAdd>1.2</VersionAdd>|Cache blocks that have not been accessed for more than the set time will be automatically cleared (even if the value of `--cache-eviction` is `none`, these cache blocks will be deleted), in seconds, a value of 0 means never expires (default: 0)|

#### Metrics related options {#metrics-options}
