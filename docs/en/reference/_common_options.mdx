...
|`--max-staging-size=0` <VersionAdd>1.4</VersionAdd>|When `--writeback` is enabled, upload staged blocks as soon as the total staging size reaches this value, regardless of `--upload-delay`, so that the amount of not-yet-uploaded data on local disk stays bounded. The value is in MiB by default and other units are supported (e.g. `1G`); 0 (the default) means no limit. If `--upload-hours` is set, uploads still only happen within the allowed time window.|
