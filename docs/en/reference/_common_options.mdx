...
|`--negative-dir-entry-cache=0` <VersionAdd>1.3, only for mount</VersionAdd>|negative lookup (return ENOENT) cache timeout in seconds (default: 0, means disable this feature)|
...
|`--cache-items=0` <VersionAdd>1.3</VersionAdd> |max number of cached items (0 for unlimited)|
