...
|`--readdir-cache=false` <VersionAdd>1.3</VersionAdd>|enable directory entry cache (default: false, disable this feature)|
|`--negative-dir-entry-cache=0` <VersionAdd>1.3</VersionAdd>|negative lookup (return ENOENT) cache timeout in seconds (default: 0, means disable this feature)|
