...
## Flexible use of internal and public Endpoint

In addition to the public Endpoint, some object storage services also provide internal Endpoint for inter-service communication between their own platforms, meeting the conditions of the cloud server using internal Endpoint access to object storage not only lower latency, and all the transport costs generated by the communication between the two services will be somewhat reduced.

:::tip
The cost of egress traffic from cloud services accessed through internal Endpoint is usually free, but the cloud platform may change the billing rules, so please refer to the latest rules published by the cloud platform you are using.
:::

JuiceFS supports flexible use of internal and public Endpoint to achieve better cost performance.  For the scenario of sharing the same file system, where the object storage is accessed through the internal Endpoint on the server that meets the conditions, and the other computers are accessed through the public Endpoint, you can use it as follows:

- **When creating a file system**: `-bucket` specifies the public Endpoint address.
- **When mounting a file system**: For cloud servers that meet the conditions, change to the internal Endpoint address via `-bucket`.

Creating a file system using the public Endpoint ensures that any networked client can mount it for access, while clients catering to internal communication can access object storage with an internal Endpoint by changing the default `--bucket` address when mounting the file system.
