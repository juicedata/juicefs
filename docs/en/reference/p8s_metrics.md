...
The `job` label indicates the specific background task, such as `cleanupSlices`, `cleanTrashFile`, or `cleanDelayedSlice`. The `status` label on the duration metric is either `success` or `failed`.
