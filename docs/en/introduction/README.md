...
JuiceFS separates "data" and "metadata" storage. Files are split into chunks and stored in [object storage](../reference/how_to_set_up_object_storage.md#supported-object-storage) like Amazon S3. The corresponding metadata can be stored in various [databases](../reference/how_to_set_up_metadata_engine.md) such as Redis, MySQL, TiKV, and SQLite. Recently, JuiceFS has also introduced support for the distributed relational database OceanBase, whose high scalability, strong consistency transaction capabilities, and high reliability make it particularly suitable for high-concurrency, large-scale metadata management scenarios. Users can choose according to the scenario and performance requirements.
