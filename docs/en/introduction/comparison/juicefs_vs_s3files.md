...
- There is an aggregation window up to 60 seconds before writes are synced back to S3.
...
- Multi-cloud and hybrid cloud support with [all main-stream object storage backends](../../reference/how_to_set_up_object_storage.md).
...
| Data storage             | S3 only                    | ✓ Any main-stream object storage                         | ✓ Any main-stream object storage storage                           |
...
- For cost-sensitive, read-heavy, write-heavy, multi-cloud scenarios, JuiceFS typically incurs lower incremental costs than S3 Files.
