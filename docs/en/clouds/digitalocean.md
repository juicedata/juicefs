# Installing and Using JuiceFS Storage on DigitalOcean
...
Here, I created a Spaces storage bucket named `juicefs` with the region `sgp1` in Singapore, and it is accessible at
...
Unlike normal file systems, JuiceFS stores all metadata corresponding to the data in a separate database, and the larger the size of the stored data, the better the performance. Currently, JuiceFS supports common databases such as Redis, TiKV, MySQL/MariaDB, PostgreSQL, SQLite, etc., while support for other databases is under continuous development. If the database you need is not supported at the moment, please submit [Issuse](https://github.com/juicedata/juicefs/issues) feedback.
...
For this article, we use DigitalOcean's Redis 6 database hosting service, choose `Singapore`, and select the same VPC private network as the existing Droplet. It takes about 5 minutes to create the Redis cluster, and we follow the setup wizard to initialize the database cluster.
...
- `--storage`: Specify the data storage engine, here is `space`, click here to view all [supported storage](https://github.com/juicedata/juicefs/blob/main/docs/en/ how_to_setup_object_storage.md).
...
> **Note**: [Force unmount the file system in use](../quick_start_guide.md#unmount-failed) may cause data damage or loss, please be careful to operate.
