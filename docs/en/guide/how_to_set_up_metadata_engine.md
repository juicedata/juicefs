...
:::note
When a Redis username or password contains special characters, you need to replace them with `%xx` by `URL encode`, for example `@` with `%40`, or pass the password using an environment variable.
