...
- Metadata engine: A component that stores and manages file system metadata, usually served by a database that supports transactions. There are three major categories of metadata engines that are currently supported.
  - Redis: Redis and various protocol-compatible services
  - SQL: MySQL, PostgreSQL, SQLite, etc.
  - TKV: TiKV, BadgerDB, etc.
- Datastore: A component used to store and manage file system data, usually served by object storage, such as Amazon S3, Aliyun OSS, etc. It can also be served by other storage systems that are compatible with object storage semantics, such as local file systems, Ceph Rados, TiKV, etc.
- Client: There are various forms, such as mount process, S3 gateway, WebDAV server, Java SDK, etc.
- File: This article refers to all types of files in general, including regular file, directory file, link file, device file, etc.
...
- Compression: compression algorithm run before uploading the data block to the object storage, the default is no compression
- Shards: the number of buckets in the object storage, the default is only one bucket; when Shards > 1, data objects will be randomly hashed into Shards buckets
...
- TrashDays: number of days after which removed files will be permanently deleted, the default is 1 day
...
- nextCleanupSlices: timestamp of the last check to clean up residual slices
- lastCleanupSessions: timestamp of the last check to clean up stale sessions
- lastCleanupFiles: timestamp of the last check to clean up residual files
- lastCleanupTrash: timestamp of the last check to clean up the trash
...
Records specific metadata about the client session so that it can be viewed with the `juicefs status` command. This is specified as
...
- Length
  ...
  - Soft link (symbolic link) file: length of the string of the path to which the link points
...
where links is the count of the parentInode, because multiple hard links can be created in the same directory, and these hard links share the inode.
...
Here plock is a more fine-grained lock that can lock only a certain segment of the file.
...
Records delayed deleted Slices. When the Trash feature is enabled, old Slices deleted by the Slice Compaction are kept for the same amount of time as the Trash configuration, to be available for data recovery if necessary.

```
sliceId, deleted -> []slice
```

where sliceId is the ID of the new slice after compaction, deleted is the timestamp of the compaction, and the mapped value is the list of all old slices that were compacted, each slice encoding only the ID and size.
