...
If the application needs to frequently delete files or frequently overwrite files, the usage of object storage will be much larger than that of the file system. Essentially, the JuiceFS client will keep the deleted files on the object storage or the data blocks that need garbage collection generated during overwriting for a period of time, unless the file is permanently deleted from the trash.

:::note
Data blocks that need garbage collection generated by overwriting cannot be forcibly deleted through the `juicefs gc` command, and must be collected after the trash retention time expires.
