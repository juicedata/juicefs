...
| juicefs-hadoop-x.x.x-linux-amd64.jar | For Hadoop on the x86 architecture  |

:::tip
For macOS systems on M1 chips, you can use the `darwin-amd64` version of the client dependent on [Rosetta 2](https://support.apple.com/zh-cn/HT211861), or you can refer to [Manually Compiling the Client](#manually-compile) to compile the native version.
:::
...
:::info
If the terminal prompts `command not found`, it may be that `/usr/local/bin` is not in your system's $PATH. You can run `echo $PATH` to see which executable paths are set, select an appropriate path based on the return result, adjust and re-execute the installation command in step 4.
:::
...
[WSL](https://docs.microsoft.com/zh-cn/windows/wsl/about) is the full name of Windows Subsystem for Linux, which is supported from Windows 10 version 2004 onwards or Windows 11. It allows you to run most of the command-line tools, utilities, and applications of GNU/Linux natively on a Windows system without incurring the overhead of a traditional virtual machine or dual-boot setup.

Because WSL is a highly complete Linux system (Ubuntu by default) running on a Windows system. Therefore, after entering the WSL subsystem terminal, you should install and use it exactly as described in [Installing and using the JucieFS client on Linux](#linux).
