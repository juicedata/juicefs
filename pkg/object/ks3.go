...
func (s *ks3) List(prefix, marker, delimiter string, limit int64, followLink bool) ([]Object, error) {
	...
	for i := 0; i < n; i++ {
		...
		if err != nil {
			return nil, errors.WithMessagef(err, "failed to decode Key_ %s", *o.Key)
		}
...
func newKS3(endpoint, accessKey, secretKey, token string) (ObjectStorage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("unescape access Key_: %s", err)
	}
	secretKey, err = url.PathUnescape(secretKey)
	if err != nil {
		return nil, fmt.Errorf("unescape secret Key_: %s", err)
	}
