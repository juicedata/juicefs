...
func (e *chunkedEncrypted) Get(key string, off, limit int64, getters ...AttrGetter) (io.ReadCloser, error) {
	...
	r, err := e.ObjectStorage.Get(key, encOff, encLimit, getters...)
	if err != nil {
		return nil, err
	}
	...
	if limit > 0 {
		return &chunkLimitedReadCloser{io.LimitReader(dr, limit), dr}, nil
	}
	return dr, nil
}

type chunkLimitedReadCloser struct {
	io.Reader
	io.Closer
}
...
func (e *chunkedEncrypted) Put(key string, in io.Reader, getters ...AttrGetter) error {
	return e.ObjectStorage.Put(key, e.newChunkEncryptReader(in), getters...)
}
...
func (e *chunkedEncrypted) Head(key string) (Object, error) {
	o, err := e.ObjectStorage.Head(key)
	if err != nil {
		return nil, err
	}
...
func (e *chunkedEncrypted) List(prefix, startAfter, token, delimiter string, limit int64, followLink bool) ([]Object, bool, string, error) {
	objs, hasMore, nextToken, err := e.ObjectStorage.List(prefix, startAfter, token, delimiter, limit, followLink)
	if err != nil {
...
func (e *chunkedEncrypted) ListAll(prefix, marker string, followLink bool) (<-chan Object, error) {
	ch, err := e.ObjectStorage.ListAll(prefix, marker, followLink)
	if err != nil {
		return nil, err
	}
...
func (e *chunkedEncrypted) UploadPart(key string, uploadID string, num int, body []byte) (*Part, error) {
	data, err := io.ReadAll(e.newChunkEncryptReader(bytes.NewReader(body)))
	if err != nil {
		return nil, err
	}
	return e.ObjectStorage.UploadPart(key, uploadID, num, data)
}

func (e *chunkedEncrypted) UploadPartCopy(key string, uploadID string, num int, srcKey string, off, size int64) (*Part, error) {
