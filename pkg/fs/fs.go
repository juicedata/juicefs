...
func (f *File) Readdir(ctx meta.Context, count int) (fi []os.FileInfo, err syscall.Errno) {
	...
	if fi == nil {
		...
		if f.fs.conf.Meta.SortDir {
			slices.SortFunc(inodes[2:], func(a, b *meta.Entry) int { return cmp.Compare(string(a.Name), string(b.Name)) })
		}
		// skip . and ..
...
func (f *File) ReaddirPlus(ctx meta.Context, offset int) (entries []*meta.Entry, err syscall.Errno) {
	...
	if f.entries == nil {
		...
		if f.fs.conf.Meta.SortDir {
			slices.SortFunc(f.entries, func(a, b *meta.Entry) int { return cmp.Compare(string(a.Name), string(b.Name)) })
		}
	}
