...
func (m *dbMeta) doMknod(ctx Context, parent Ino, name string, _type uint8, mode, cumask uint16, rdev uint32, path string, inode *Ino, attr *Attr) syscall.Errno {
	...
	return errno(m.txn(func(s *xorm.Session) error {
		...
		if parent != TrashInode {
			...
			if updateParent || time.Duration(now-pn.Mtime*1e3-int64(pn.Mtimensec)) >= m.conf.MinUpdateTime {
				pn.Mtime = now / 1e3
				pn.Ctime = now / 1e3
				updateParent = true
			}
...
func (m *dbMeta) doUnlink(ctx Context, parent Ino, name string, attr *Attr, skipCheckTrash ...bool) syscall.Errno {
	...
	err := m.txn(func(s *xorm.Session) error {
		...
		var updateParent bool
		if !isTrash(parent) && time.Duration(now-pn.Mtime*1e3-int64(pn.Mtimensec)) >= m.conf.MinUpdateTime {
			pn.Mtime = now / 1e3
			pn.Ctime = now / 1e3
...
func (m *dbMeta) doRename(ctx Context, parentSrc Ino, nameSrc string, parentDst Ino, nameDst string, flags uint32, inode, tInode *Ino, attr, tAttr *Attr) syscall.Errno {
	...
	err := m.txn(func(s *xorm.Session) error {
		...
		if supdate || time.Duration(now-spn.Mtime*1e3-int64(spn.Mtimensec)) >= m.conf.MinUpdateTime {
			spn.Mtime = now / 1e3
			spn.Ctime = now / 1e3
		...
		if dupdate || time.Duration(now-dpn.Mtime*1e3-int64(dpn.Mtimensec)) >= m.conf.MinUpdateTime {
			dpn.Mtime = now / 1e3
			dpn.Ctime = now / 1e3
...
func (m *dbMeta) doLink(ctx Context, inode, parent Ino, name string, attr *Attr) syscall.Errno {
	return errno(m.txn(func(s *xorm.Session) error {
		...
		var updateParent bool
		now := time.Now().UnixNano()
		if time.Duration(now-pn.Mtime*1e3-int64(pn.Mtimensec)) >= m.conf.MinUpdateTime {
			pn.Mtime = now / 1e3
			pn.Ctime = now / 1e3
			updateParent = true
		}
