...
func Apply(ctx Context, dst Meta, e *ChangeEntry) (err error) {
	ctx = ctx.WithValue(applyStateKey{}, &applyState{Time: e.Time, Sid: e.Sid})
	...
	case OpIncrCounter:
		...
		if name == usedSpace || name == totalInodes {
			// FIXME: Rebuild destination volume stats before apply; backup counters may
			// exclude source updates that had not been flushed when the backup was taken.
...
func applyLink(ctx Context, dst Meta, e *ChangeEntry) error {
	...
	getApplyState(ctx).Parents = map[Ino]bool{parent: updateParent}
	var attr Attr
...
func applyRmdir(ctx Context, dst Meta, e *ChangeEntry) error {
	...
	getApplyState(ctx).Trash = Ino(trash)
...
func applyMove(ctx Context, dst Meta, e *ChangeEntry) error {
	...
	getApplyState(ctx).Trash = trash
	var inode Ino
	// FIXME: MOVE lacks parent-update decisions and the overwritten inode's
	// open state; parent timestamps and sustained inode handling can diverge.
