...
func testRace(t *testing.T, m Meta) {
	...
	t.Run("SQLNamespace", func(t *testing.T) {
		db, ok := m.(*dbMeta)
		if !ok {
			t.Skip("SQL transaction invariant")
		}
		t.Run("ParentLockRevalidation", func(t *testing.T) {
			testSQLNamespaceNodeLockRevalidatesParent(t, db)
		})
	})
}

func testSQLNamespaceNodeLockRevalidatesParent(t *testing.T, m *dbMeta) {
	err := m.txn(func(s *xorm.Session) error {
		parent := node{Inode: RootInode}
		ok, err := s.Get(&parent)
		if err != nil {
			return err
		}
		...
		if n, err := s.Delete(&node{Inode: parent.Inode}); err != nil {
			return err
		} else if n != 1 {
			return syscall.ENOENT
		}
		_, err = m.lockNamespaceNodes(s, sharedNamespaceNode(&parent))
		return err
	})
	if !errors.Is(err, syscall.ENOENT) {
		t.Fatalf("lock deleted parent: got %v, want %v", err, syscall.ENOENT)
	}
	var root node
	if ok, err := m.db.ID(RootInode).Get(&root); err != nil || !ok {
		t.Fatalf("parent deletion was not rolled back: found=%t err=%v", ok, err)
	}
}
...
func testTrash(t *testing.T, m Meta) {
	...
	// removing a subTrash directory must invalidate the local cache
	base := m.getBase()
	base.Lock()
	cached := base.subTrash
	base.Unlock()
	if cached.inode != 0 || cached.name != "" {
		t.Fatalf("subTrash cache was not invalidated: %+v", cached)
	}

	// simulate a stale cache pointing to a subTrash directory removed by
	// another client: operations should delete entries directly instead of
	// orphaning them under the removed directory
	setStaleSubTrash := func() {
		base.Lock()
		base.subTrash = internalNode{inode: TrashInode + 1, name: time.Now().UTC().Format("2006-01-02-15")}
		base.Unlock()
	}
	defer func() {
		base.Lock()
		base.subTrash = internalNode{}
		base.Unlock()
	}()

	setStaleSubTrash()
	if st := m.Mknod(ctx, 1, "stale_f", TypeFile, 0644, 022, 0, "", &inode, attr); st != 0 {
		t.Fatalf("mknod stale_f: %s", st)
	}
	if st := m.Unlink(ctx, 1, "stale_f"); st != 0 {
		t.Fatalf("unlink stale_f: %s", st)
	}
	if st := m.GetAttr(ctx, inode, attr); st != syscall.ENOENT {
		t.Fatalf("stale_f should be deleted directly, got %s", st)
	}

	setStaleSubTrash()
	if st := m.Mkdir(ctx, 1, "stale_d", 0755, 022, 0, &inode, attr); st != 0 {
		t.Fatalf("mkdir stale_d: %s", st)
	}
	if st := m.Rmdir(ctx, 1, "stale_d"); st != 0 {
		t.Fatalf("rmdir stale_d: %s", st)
	}
	if st := m.GetAttr(ctx, inode, attr); st != syscall.ENOENT {
		t.Fatalf("stale_d should be deleted directly, got %s", st)
	}

	setStaleSubTrash()
	if st := m.Mknod(ctx, 1, "stale_r1", TypeFile, 0644, 022, 0, "", &inode, attr); st != 0 {
		t.Fatalf("mknod stale_r1: %s", st)
	}
	var replaced Ino
	if st := m.Mknod(ctx, 1, "stale_r2", TypeFile, 0644, 022, 0, "", &replaced, attr); st != 0 {
		t.Fatalf("mknod stale_r2: %s", st)
	}
	if st := m.Rename(ctx, 1, "stale_r1", 1, "stale_r2", 0, &inode, attr); st != 0 {
		t.Fatalf("rename stale_r1 -> stale_r2: %s", st)
	}
	if st := m.GetAttr(ctx, replaced, attr); st != syscall.ENOENT {
		t.Fatalf("replaced stale_r2 should be deleted directly, got %s", st)
	}
	if st := m.Unlink(ctx, 1, "stale_r2"); st != 0 {
		t.Fatalf("unlink stale_r2: %s", st)
	}

	setStaleSubTrash()
	var sb1, sb2 Ino
	var sbAttr1, sbAttr2 Attr
	if st := m.Mknod(ctx, 1, "stale_b1", TypeFile, 0644, 022, 0, "", &sb1, &sbAttr1); st != 0 {
		t.Fatalf("mknod stale_b1: %s", st)
	}
	if st := m.Mknod(ctx, 1, "stale_b2", TypeFile, 0644, 022, 0, "", &sb2, &sbAttr2); st != 0 {
		t.Fatalf("mknod stale_b2: %s", st)
	}
	staleBatch := []*Entry{
		{Inode: sb1, Name: []byte("stale_b1"), Attr: &sbAttr1},
		{Inode: sb2, Name: []byte("stale_b2"), Attr: &sbAttr2},
	}
	if st := m.BatchUnlink(ctx, 1, staleBatch, nil, false); st != 0 {
		t.Fatalf("batch unlink stale_b1/stale_b2: %s", st)
	}
	if st := m.GetAttr(ctx, sb1, attr); st != syscall.ENOENT {
		t.Fatalf("stale_b1 should be deleted directly, got %s", st)
	}
	if st := m.GetAttr(ctx, sb2, attr); st != syscall.ENOENT {
		t.Fatalf("stale_b2 should be deleted directly, got %s", st)
	}
}
