...
func testMetaClient(t *testing.T, m Meta) {
	...
	if st := m.Remove(ctx, 1, "d1", false, 50, nil); st != 0 {
...
func testResolve(t *testing.T, m Meta) {
	...
	defer func() {
		if st := m.Remove(NewContext(0, 65534, []uint32{65534}), parent, "f", false, 50, nil); st != 0 {
...
func testRemove(t *testing.T, m Meta) {
	...
	if st := m.Remove(ctx, 1, "f", false, 50, nil); st != 0 {
		t.Fatalf("rmr f: %s", st)
	}
	...
	if st := m.Remove(ctx, 1, "d", false, 50, nil); st != 0 {
		t.Fatalf("rmr d: %s", st)
	}
}
...
func testTrash(t *testing.T, m Meta) {
	...
	if st := m.Remove(ctx, 1, "d10", false, 50, nil); st != 0 {
	...
	if st := m.Remove(ctx, 1, "d10", true, 50, nil); st != 0 {
		t.Fatalf("rmr d10: %s", st)
	}
	entries = entries[:0]
	if st := m.Readdir(ctx, TrashInode+1, 0, &entries); st != 0 {
...
func testClone(t *testing.T, m Meta) {
	...
	if eno := m.Remove(Background(), cloneDir, "no_preserve", false, 50, nil); eno != 0 {
	...
	if eno := m.Remove(Background(), cloneDir, "dir1", false, 50, nil); eno != 0 {
		t.Fatalf("Rmdir: %s", eno)
	}
