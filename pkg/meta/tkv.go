...
func (m *kvMeta) get(key []byte) ([]byte, error) {
	var value []byte
	var err error
	m.client.txn(func(tx *kvTxn) error {
...
func (m *kvMeta) doInit(format *Format, force bool) error {
	...
	return m.txn(func(tx *kvTxn) error {
		if format.TrashDays > 0 {
			buf, err := tx.get(m.inodeKey(TrashInode))
			if err != nil {
				return err
			}
			if buf == nil {
...
func (m *kvMeta) doRefreshSession() error {
	return m.txn(func(tx *kvTxn) error {
		buf, err := tx.get(m.sessionKey(m.sid))
		if err != nil {
			return err
		}
		if buf == nil {
...
func (m *kvMeta) Truncate(ctx Context, inode Ino, flags uint8, length uint64, attr *Attr, skipPermCheck bool) syscall.Errno {
	...
	err := m.txn(func(tx *kvTxn) error {
		...
		a, xerr := tx.get(m.inodeKey(inode))
		if xerr != nil {
			return xerr
		}
		if a == nil {
...
func (m *kvMeta) doReadlink(ctx Context, inode Ino, noatime bool) (atime int64, target []byte, err error) {
	...
	now := time.Now()
	err = m.txn(func(tx *kvTxn) error {
		rs, xerr := tx.gets(m.inodeKey(inode), m.symKey(inode))
		if xerr != nil {
			return xerr
		}
...
func (m *kvMeta) doMknod(ctx Context, parent Ino, name string, _type uint8, mode, cumask uint16, rdev uint32, path string, inode *Ino, attr *Attr) syscall.Errno {
	...
	return errno(m.txn(func(tx *kvTxn) error {
		var pattr Attr
		a, xerr := tx.get(m.inodeKey(parent))
		if xerr != nil {
			return xerr
		}
		if a == nil {
		...
		buf, xerr := tx.get(m.entryKey(parent, name))
		if xerr != nil {
			return xerr
		}
		var foundIno Ino
		...
		if foundIno != 0 {
			if _type == TypeFile || _type == TypeDirectory {
				a, xerr = tx.get(m.inodeKey(foundIno))
				if xerr != nil {
					return xerr
				}
				if a != nil {
...
func (m *kvMeta) doRmdir(ctx Context, parent Ino, name string, pinode *Ino, skipCheckTrash ...bool) syscall.Errno {
	...
	err := m.txn(func(tx *kvTxn) error {
		...
		rs, xerr := tx.gets(m.inodeKey(parent), m.inodeKey(inode))
		if xerr != nil {
			return xerr
		}
		if rs[0] == nil {
...
func (m *kvMeta) doRename(ctx Context, parentSrc Ino, nameSrc string, parentDst Ino, nameDst string, flags uint32, inode, tInode *Ino, attr, tAttr *Attr) syscall.Errno {
	...
	err := m.txn(func(tx *kvTxn) error {
		...
		tattr = Attr{}
		newSpace, newInode = 0, 0
		buf, xerr := tx.get(m.entryKey(parentSrc, nameSrc))
		if xerr != nil {
			return xerr
		}
		if buf == nil && m.conf.CaseInsensi {
		...
		rs, xerr := tx.gets(m.inodeKey(parentSrc), m.inodeKey(parentDst), m.inodeKey(ino))
		if xerr != nil {
			return xerr
		}
		...
		dbuf, xerr := tx.get(m.entryKey(parentDst, nameDst))
		if xerr != nil {
			return xerr
		}
...
func (m *kvMeta) Write(ctx Context, inode Ino, indx uint32, off uint32, slice Slice, mtime time.Time) syscall.Errno {
	...
	err := m.txn(func(tx *kvTxn) error {
		newLength, newSpace = 0, 0
		attr = Attr{}
		rs, xerr := tx.gets(m.inodeKey(inode), m.chunkKey(inode, indx))
		if xerr != nil {
			return xerr
		}
		if rs[0] == nil {
...
func (m *kvMeta) CopyFileRange(ctx Context, fin Ino, offIn uint64, fout Ino, offOut uint64, size uint64, flags uint32, copied *uint64) syscall.Errno {
	...
	err := m.txn(func(tx *kvTxn) error {
		newLength, newSpace = 0, 0
		rs, xerr := tx.gets(m.inodeKey(fin), m.inodeKey(fout))
		if xerr != nil {
			return xerr
		}
		if rs[0] == nil || rs[1] == nil {
...
func (m *kvMeta) doUpdateDirStat(ctx Context, batch map[Ino]dirStat) error {
	...
	for _, group := range m.groupBatch(batch, 20) {
		err := m.txn(func(tx *kvTxn) error {
			...
			xv, xerr := tx.gets(keys...)
			if xerr != nil {
				return xerr
			}
			for i, rawStat := range xv {
				ino := group[i]
				if rawStat == nil {
					syncMap[ino] = true
					continue
...
func (m *kvMeta) deleteChunk(inode Ino, indx uint32) error {
	...
	err := m.txn(func(tx *kvTxn) error {
		...
		for _, s := range slices {
			xv, xerr := tx.incrBy(m.sliceKey(s.id, s.size), -1)
			if xerr != nil {
				return xerr
			}
			if s.id > 0 && xv < 0 {
...
func (r *kvMeta) cleanupZeroRef(id uint64, size uint32) {
	_ = r.txn(func(tx *kvTxn) error {
		v, err := tx.incrBy(r.sliceKey(id, size), 0)
		if err != nil {
			return err
		}
		if v != 0 {
...
func (m *kvMeta) compactChunk(inode Ino, indx uint32, force bool) {
	...
	err = m.txn(func(tx *kvTxn) error {
		buf2, xerr := tx.get(m.chunkKey(inode, indx))
		if xerr != nil {
			return xerr
		}
		...
		if !trash {
			...
			for _, s := range ss {
				if s.id > 0 && m.client.txn(func(tx *kvTxn) error {
					var xerr error
					refs, xerr = tx.incrBy(m.sliceKey(s.id, s.size), 0)
					return xerr
				}, 0) == nil && refs < 0 {
					m.deleteSlice(s.id, s.size)
				}
...
func (m *kvMeta) doSetQuota(ctx Context, inode Ino, quota *Quota, create bool) error {
	return m.txn(func(tx *kvTxn) error {
		if create {
			...
			buf, xerr := tx.get(m.dirQuotaKey(inode))
			if xerr != nil {
				return xerr
			}
			if len(buf) != 32 {
...
func (m *kvMeta) DumpMeta(w io.Writer, root Ino, keepSecret bool) (err error) {
	...
	err = m.txn(func(tx *kvTxn) error {
		var xerr error
		rs, xerr = tx.gets(m.counterKey(usedSpace),
...
func (m *kvMeta) doCloneEntry(ctx Context, srcIno Ino, parent Ino, name string, ino Ino, attr *Attr, cmode uint8, cumask uint16, top bool) syscall.Errno {
	return errno(m.txn(func(tx *kvTxn) error {
		a, xerr := tx.get(m.inodeKey(srcIno))
		if xerr != nil {
			return xerr
		}
		if a == nil {
		...
		if top {
			var err error
			var pattr Attr
			a, err = tx.get(m.inodeKey(parent))
			if err != nil {
				return err
			}
			if a == nil {
			...
			v, xerr := tx.get(m.entryKey(parent, name))
			if xerr != nil {
				return xerr
			}
			if v != nil {
		...
		switch attr.Typ {
		case TypeDirectory:
			xv, xerr := tx.get(m.dirStatKey(srcIno))
			if xerr != nil {
				return xerr
			}
			tx.set(m.dirStatKey(ino), xv)
		case TypeFile:
			if attr.Length != 0 {
				vals := make(map[string][]byte)
		...
		case TypeSymlink:
			xv, xerr := tx.get(m.symKey(srcIno))
			if xerr != nil {
				return xerr
			}
			tx.set(m.symKey(ino), xv)
		}
		return nil
	}, srcIno))
}
...
func (m *kvMeta) doAttachDirNode(ctx Context, parent Ino, inode Ino, name string) syscall.Errno {
	return errno(m.txn(func(tx *kvTxn) error {
		...
		xv, xerr := tx.get(m.entryKey(parent, name))
		if xerr != nil {
			return xerr
		}
		if xv != nil {
