...
func TestPreRead(t *testing.T) {
	key := "testPreRead"
	a, _ := object.CreateStorage("file", "/tmp/preRead/", "", "", "")
	t.Cleanup(func() {
		os.RemoveAll("/tmp/preRead/")
	})
	type config struct {
		blockSize  int64
		concurrent int
	...
	type tcase struct {
		config
		tfunc func(t *testing.T, pr *preRead, content []byte)
	}

	tcases := []tcase{
		{config: config{fsize: 1110, concurrent: 4, blockSize: 300}, tfunc: func(t *testing.T, pr *preRead, content []byte) {
		...
		{config: config{fsize: 97340326, concurrent: 4, blockSize: 5 << 20}, tfunc: func(t *testing.T, pr *preRead, content []byte) {
			res, err := io.ReadAll(pr)
			if err != nil {
				t.Fatal(err)
			}
		...
		{config: config{fsize: 1110, concurrent: 5, blockSize: 300}, tfunc: func(t *testing.T, pr *preRead, content []byte) {
			res, err := io.ReadAll(pr)
			if err != nil {
				t.Fatal(err)
			}
		...
		{config: config{fsize: 1, concurrent: 5, blockSize: 10}, tfunc: func(t *testing.T, pr *preRead, content []byte) {
			res := make([]byte, 1)
			n, err := pr.Read(res)
		...
		{config: config{fsize: 2, concurrent: 5, blockSize: 10}, tfunc: func(t *testing.T, pr *preRead, content []byte) {
			res := make([]byte, 1)
			n, err := pr.Read(res)
			if err != nil || n != 1 || res[0] != content[0] {
				t.Fatalf("read 1 byte should succeed")
		...
		{config: config{fsize: 2, concurrent: 1, blockSize: 10}, tfunc: func(t *testing.T, pr *preRead, content []byte) {
			res := make([]byte, 1)
			n, err := pr.Read(res)
	...
	for _, c := range tcases {
		...
		c.tfunc(t, &preRead{key: key, src: a, blockSize: c.blockSize, concurrent: c.concurrent, fsize: c.config.fsize}, content)
	}

	writeCase := []config{
		{fsize: 100, blockSize: 40, concurrent: 2},
		{fsize: 100, blockSize: 50, concurrent: 2},
		{fsize: 100, blockSize: 30, concurrent: 3},
		{fsize: 1000, blockSize: 50, concurrent: 4},
		{fsize: 10, blockSize: 100, concurrent: 2},
		{fsize: 0, blockSize: 100, concurrent: 2},
	}
	for _, c := range writeCase {
		content := make([]byte, c.fsize)
		rand.Read(content)
		_ = a.Put(key, bytes.NewReader(content))
		pr := &preRead{key: key, src: a, blockSize: c.blockSize, concurrent: c.concurrent, fsize: c.fsize}
		var buf bytes.Buffer
		n, err := pr.WriteTo(&buf) //nolint:typecheck
		if err != nil || n != pr.fsize {
