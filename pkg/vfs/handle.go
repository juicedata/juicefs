...
func (v *VFS) newFileHandle(inode Ino, length uint64, flags uint32, tier meta.TierInfo) uint64 {
	...
	case syscall.O_RDWR:
		h.reader = v.reader.Open(inode, length)
		h.writer = v.writer.Open(inode, length, tier)
	}
	return h.fh
}
...
func (v *VFS) loadAllHandles(path string) error {
	...
	for fh, s := range vfsState.Handler {
		...
		case syscall.O_RDWR:
			...
			h.writer = v.writer.Open(h.inode, s.Length, meta.TierInfo(0))
		}
	}
