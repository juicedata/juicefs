...
func mountSimpleMethod(args []string,flagMap map[string]string) {
	...
	chunkConf := chunk.Config{
		...
		//GetTimeout:  time.Second * time.Duration(60),
		//PutTimeout:  time.Second * time.Duration(60),
		//MaxUpload:   20,
	...
	xattrFlags,convErr := strconv.ParseBool(flagMap["enable-xattr"])
	if err != nil {
		log.Fatalf("close session err: %s\n", convErr)
	}
	serverErr := fuse.Serve(conf, "", xattrFlags)
	if serverErr != nil {
