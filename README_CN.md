...
- **共享**: 可以被多个客户端同时读写；
- **文件锁**: 支持 BSD 锁 (flock) and POSIX 锁 (fcntl).
- **数据压缩**: 使用默认的 [LZ4](https://lz4.github.io/lz4) 压缩数据，节省存储空间。

---

[架构](#architecture) | [开始使用](#getting-started) | [POSIX 兼容性](#posix-compatibility) | [性能测试](#performance-benchmark) | [支持的对象存储](#supported-object-storage) | [状态](#status) | [路线图](#roadmap) | [反馈](#reporting-issues) | [贡献](#contributing) | [社区](#community) | [使用量收集](#usage-tracking) | [开源协议](#license) | [致谢](#credits) | [FAQ](#faq)
...
JuiceFS 中的文件格式，如下图所示。一个文件首先被拆分成固定大小的 **"Chunk"**，默认 64 MiB。每个 Chunk 可以由一个或者多个 **"Slice"** 组成，它们是变长的。对于每一个 Slice，又会被拆分成固定大小的 **"Block"**，默认为 4 MiB（格式化就不可以修改）。最后，这些 Block 会把压缩和加密保存到对象存储中。压缩和加密都是可选的。
...
需要 Redis (2.2以上) 服务器来存储元数据，请参考 [Redis Quick Start](https://redis.io/topics/quickstart)。
...
上图显示 JuiceFS 可以比其他两者提供10倍以上的吞吐，详细结果请看[这里](docs/fio.md).
...
我们使用 [GitHub Issues](https://github.com/juicedata/juicefs/issues) 来管理社区反馈的问题，你也可以通过其他[渠道](#community)跟社区联系。
...
使用开源协议 GNU AGPL v3.0, 详见 [LICENSE](LICENSE).

## 致谢

JuiceFS 的设计参考了 [Google File System](https://research.google/pubs/pub51), [HDFS](https://hadoop.apache.org) and [MooseFS](https://moosefs.com)，感谢他们的杰出工作。

## FAQ

### 为什么不支持某个对象存储?

已经支持了绝大部分对象存储，参考这个[列表](#supported-object-storage)。如果它跟 S3 兼容的话，也可以当成 S3 来使用。否则，请到 juicesync 创建一个 issue 来增加支持。

### 是否可以使用 Redis 集群版?

不可以。JuiceFS 使用了 Redis 的[事务功能](https://redis.io/topics/transactions) 来保证元数据操作的原子性，而分布式版还不支持多机事务。
