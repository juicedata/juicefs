...
public class JuiceFileSystemImpl extends FileSystem {
  ...
  public void initialize(URI uri, Configuration conf) throws IOException {
    ...
    if (withStreamCapability) {
      try {
        constructor = Class.forName("com.juicefs.JuiceFileSystemImpl$BufferedFSOutputStreamWithStreamCapabilities")
  ...
  private FileSystem getFileSystem(Path path) throws IOException {
    ...
    if (callerContext != null && !"".equals(callerContext.trim())) {
      try {
        Class<?> ctx = Class.forName("com.juicefs.utils.CallerContextUtil");
