...
func getOrCreate(name, user, group, superuser, supergroup, jConf string, superFs bool, f func() *fs.FileSystem) int64 {
	fslock.Lock()
	defer fslock.Unlock()
	ws := activefs[name]
	...
	w := &wrapper{jfs, nil, m, user, superuser, supergroup, superFs, jConf}
	var gs []string
...
func jfs_init(cname, cjsonConf, user, group, superuser, supergroup *C.char) int64 {
	...
	return getOrCreate(name, C.GoString(user), C.GoString(group), C.GoString(superuser), C.GoString(supergroup), jsonConf, jConf.SuperFS, func() *fs.FileSystem {
