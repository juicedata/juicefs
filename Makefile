...
test.meta:
	go test -v -cover -count=1  -failfast -timeout=12m ./pkg/meta/... -coverprofile=cov.out

test.object:
	go test -v -cover -count=1  -failfast -timeout=12m ./pkg/object/... -coverprofile=cov.out

test.pkg:
	go test -v -cover -count=1  -failfast -timeout=12m $(go list ./pkg/... | grep -v /object | grep -v /meta) -coverprofile=cov.out
