...
# official writer (pylance, see scripts/gen-fixtures.py) and verify that the
...
# content lands in which file. scripts/dump_fixture.py normalizes all of that, so
# the JSON is stable across regenerations (verified empirically).
#
# Usage: tools/lance-resolver/scripts/check-fixture-drift.sh
...
python "${SCRIPT_DIR}/gen-fixtures.py" "${TMP_DIR}/lance-dataset"
python "${SCRIPT_DIR}/dump_fixture.py" "${TMP_DIR}/lance-dataset" "${TMP_DIR}/expected.json"
...
if git diff --no-index --exit-code testdata/expected.json "${TMP_DIR}/expected.json"; then
  ...
  echo "  python scripts/gen-fixtures.py && python scripts/dump_fixture.py testdata/lance-dataset testdata/expected.json" >&2
  exit 1
fi
